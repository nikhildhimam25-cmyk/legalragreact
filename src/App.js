...
:root{
  --bg:#f5f4f0;--sidebar-bg:#efefeb;--sidebar-border:#e0dfd8;
  --text:#1a1915;--text-muted:#6b6a63;--text-light:#9e9d96;
  --accent:#c96b3a;--accent-light:#f0e8e0;
  --bubble-user:#1a1915;--bubble-ai:#ffffff;
  --bubble-user-text:#f5f4f0;--bubble-ai-text:#1a1915;
...
.pill{padding:3px 10px;background:var(--accent-light);color:var(--accent);border-radius:20px;font-size:11.5px;font-weight:500;border:1px solid rgba(201,107,58,.2)}
.typing-dots{display:flex;gap:5px;padding:14px 16px;background:var(--bubble-ai);border:1px solid var(--border);border-radius:14px;border-bottom-left-radius:4px;align-items:center;width:fit-content;box-shadow:var(--shadow)}
.typing-dots span{width:7px;height:7px;border-radius:50%;background:var(--text-muted);animation:bounce 1.1s infinite}
...
export default function App() {
  ...
  const sendMessage = async () => {
    ...
    if (session?.name === "New Chat") {
      setSessions((prev) =>
        prev.map((s) =>
          s.id === sid ? { ...s, name: question.slice(0, 32) + (question.length > 32 ? "…" : "") } : s
        )
      );
    }
    setLoading(true);
    try {
      ...
      updateMessages(sid, (msgs) => [
        ...msgs,
        { role: "ai", text: data.answer || data.response || "No response.", sources: data.sources || [] },
      ]);
    } catch {
      updateMessages(sid, (msgs) => [
        ...msgs,
  ...
  const uploadFile = async (e, endpoint, label) => {
    ...
    try {
      ...
      if (endpoint === "/upload-pdf") {
        setSessions((prev) =>
          prev.map((s) => s.id === sid ? { ...s, name: file.name.replace(".pdf","").slice(0,32) } : s)
        );
        updateMessages(sid, (msgs) => [
          ...msgs,
          { role: "ai", text: `✅ PDF uploaded! ${data.chunks || ""} chunks indexed. Ask me anything about it.`, sources: [] },
        ]);
      } else {
        updateMessages(sid, (msgs) => [
          ...msgs,
